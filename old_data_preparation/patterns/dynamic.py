...
This module implements pattern discovery for scenarios where all variables
(Ore, WaterMill, WaterZumpf) are changing, capturing dynamic operational states.
"""
import logging
...
@PatternRegistry.register('dynamic')
class DynamicPattern(BasePattern):
    """
    Discovers patterns where all variables (Ore, WaterMill, WaterZumpf) are changing.
    
    This pattern is useful for identifying dynamic operational states where the
    system is undergoing coordinated changes across multiple variables.
    """
    
    def __init__(self, config: Dict[str, Any]):
        """
        Initialize the dynamic pattern discovery.
    ...
    def discover(self, data: pd.DataFrame) -> List[Motif]:
        """
        Discover dynamic patterns in the data.
        ...
        """
        logger.info("Discovering dynamic patterns...")
        ...
        motifs = self._find_dynamic_patterns(
            data, T, matrix_profile, profile_indices, mp_distances
        )
        
        self.motifs = motifs
        logger.info(f"Discovered {len(motifs)} dynamic motifs")
        return motifs
    
    def analyze(self, data: pd.DataFrame) -> Dict[str, Any]:
        """
        Analyze the discovered dynamic patterns.
    ...
    def _find_dynamic_patterns(
        ...
        """Find patterns that match the dynamic constraints."""
        ...
        for _ in range(self.max_motifs):
            ...
            seed_idx = self._find_dynamic_seed(df, mp_distances, used_indices, n_windows)
            if seed_idx is None or mp_distances[seed_idx] > self.radius:
                break
                
            # Find similar instances
            instances = self._find_dynamic_instances(
    ...
    def _find_dynamic_seed(
        ...
        """Find the best seed index that meets the dynamic constraints."""
        best_idx = None
    ...
    def _check_variability_constraints(self, df: pd.DataFrame, idx: int) -> bool:
        """Check if the window at idx meets the dynamic variability constraints."""
        ...
        # Check constraints - all variables must be changing
        return (ore_cv >= self.ore_min_cv and
                watermill_cv >= self.watermill_min_cv and
                waterzumpf_cv >= self.waterzumpf_min_cv)
    
    def _find_dynamic_instances(
        self,
        df: pd.DataFrame,
        T: np.ndarray,
        seed_idx: int,
        ...
        """Find all instances of the dynamic pattern starting at seed_idx."""
        instances = []
    ...
    def _plot_motif(self, motif: Motif, output_path: Path) -> None:
        ...
        plt.suptitle(f'Dynamic Motif {motif.motif_id} - {len(motif.instances)} instances')
        plt.tight_layout()
